...
AS $$
  ...
  candidate_customers AS (
    ...
    OR EXISTS (
      SELECT 1
      FROM "to_historis" th
      WHERE th."idPelanggan" = p."idPelanggan"
    )
    OR EXISTS (
      SELECT 1
      FROM latest_operational_label lol
  ...
  upserted AS (
    ...
    RETURNING "pelanggan_id"
  )
  DELETE FROM "ml_customer_features" mcf
  WHERE NOT EXISTS (
    SELECT 1
    FROM candidate_customers cc
    WHERE cc."id" = mcf."pelanggan_id"
  );
$$;
